...
Admins can pay drivers directly from the admin panel via two methods:
1. **Bank Transfer**: The existing multi-step flow shows driver bank details, accepts an amount, confirms the payment, and records it manually. Company bank details for Run Courier are stored in `data/company-settings.json`.
2. **Card Payment**: Uses Stripe Elements embedded in the payment dialog. Creates a PaymentIntent via `POST /api/admin/driver-payments/create-payment-intent`, processes the card payment client-side, then confirms and records via `POST /api/admin/driver-payments/confirm-card-payment`. The confirm endpoint validates the PaymentIntent metadata (type, driverId, currency) server-side and derives the amount from Stripe rather than trusting client input. Idempotency is enforced by checking for duplicate `payoutReference` values.

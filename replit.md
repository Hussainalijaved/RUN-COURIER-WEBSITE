...
### Fleet File (Local Backup)
The Fleet File system (`/admin/fleet-file`) creates a local backup of all driver data independent of Supabase. When synced, it saves all driver profiles, applications, and document metadata to `data/fleet/` as JSON files. This includes a master file (`fleet-master.json`), individual driver files in `data/fleet/drivers/`, and a sync log. The backup persists on the hosting server (Hostinger) and can be downloaded as a JSON file. API endpoints: `POST /api/admin/fleet-file/sync`, `GET /api/admin/fleet-file/status`, `GET /api/admin/fleet-file/drivers`, `GET /api/admin/fleet-file/driver/:id`, `GET /api/admin/fleet-file/download`.
